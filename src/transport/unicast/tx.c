...
static z_result_t _z_unicast_transport_peer_flush_buffer(_z_unicast_transport_peer_t *peer, _z_wbuf_t *wbuf) {
    ...
    z_result_t ret = _Z_RES_OK;
    if (!_z_unicast_link_send_wbuf(&peer->_link, wbuf)) {
        ret = _Z_ERR_TRANSPORT_TX_FAILED;
        _Z_ERROR_LOG(ret);
    } else {
...
static z_result_t _z_unicast_transport_peer_send_fragment(_z_unicast_transport_peer_t *peer, _z_wbuf_t *tx_buf,
    ...
    while (_z_wbuf_len(&frag_buff) > 0) {
        ...
        ret = _z_unicast_transport_peer_flush_buffer(peer, tx_buf);
        if (ret != _Z_RES_OK) {
...
z_result_t _z_unicast_transport_manager_send_n_msg_to_peer(_z_unicast_transport_manager_t *manager,
    ...
    _z_unicast_transport_peer_t *peer =
        _z_unicast_transport_peer_hset_at(&manager->_peers, (_z_unicast_transport_peer_hset_iter_t)peer_id);
    _z_zint_t *sn = _z_sn_get(&peer->_sn_tx, reliability);
    ...
    if (!new_frame && peer->_is_current_tx_frame_reliable != (reliability == Z_RELIABILITY_RELIABLE)) {
        ...
        _Z_CLEAN_RETURN_IF_ERR(_z_unicast_transport_peer_flush_buffer(peer, tx_buf), _z_wbuf_reset(tx_buf));
        new_frame = true;
    }
...
#if Z_FEATURE_FRAGMENTATION == 1
    ret = _z_unicast_transport_peer_send_fragment(peer, tx_buf, msg, reliability);
#endif
    return ret;
}
...
z_result_t _z_unicast_transport_manager_send_n_msg(_z_unicast_transport_manager_t *manager,
    ...
    if (_z_destination_filter_is_what(&dest_filter)) {
        ...
        _z_unicast_transport_peer_t *peer = NULL;
        _ZP_FOREACH (_z_unicast_transport_peer_hset, &manager->_peers, peer) {
            ...
            if (ret != _Z_RES_OK) {
                _Z_ERROR("Failed to send message to unicast peer %zu with err %d", (size_t)peer_iter, ret);
            }
        }
...
z_result_t _z_unicast_transport_manager_send_t_msg_to_peer(_z_unicast_transport_manager_t *manager,
    ...
    if (_z_wbuf_len(tx_buf) > 0) {
        _Z_RETURN_IF_ERR(_z_unicast_transport_peer_flush_buffer(peer, tx_buf));
    }
...
z_result_t _z_unicast_transport_manager_send_n_batch(_z_unicast_transport_manager_t *manager) {
    ...
    _ZP_FOREACH (_z_unicast_transport_peer_hset, &manager->_peers, peer) {
        if (_z_wbuf_len(&peer->_tx_buffer) > 0) {
            ...
            if (ret != _Z_RES_OK) {
                _Z_ERROR("Failed to flush peer %zu buffer with err %d",
                         (size_t)_z_unicast_transport_peer_hset_get_iter(&manager->_peers, peer), ret);
                res = ret;
            }
        }
    }
